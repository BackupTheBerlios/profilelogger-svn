...
Bed* Profile::createBed(int id, int position) {
    if (id != 0) {
        if (id > _lastBedId) {
            _lastBedId = id;
        }
    ...
    if (position > 0) {
        _beds.insert(position, new Bed(this, _lastBedId, position, QString::number(position)));
    } else {
        _beds.append(new Bed(this, _lastBedId, position, QString::number(position)));
    }

    renumberBeds();
    Bed* b = getBedById(_lastBedId);
...
Sample* Profile::createSample(int id) {
    if (id != 0) {
        if (id > _lastSampleId) {
            _lastSampleId = id;
        }
    ...
    _samples.append(new Sample(_lastSampleId, QObject::tr("New Sample"), QString::null, this));
    return getSample(_lastSampleId);
}
...
Image* Profile::createImage(int id) {
    if (id != 0) {
        if (id > _lastImageId) {
            _lastImageId = id;
        }
    ...
    _images.append(new Image(_lastImageId, QObject::tr("New Image"), QString::null));
    return getImage(_lastImageId);
}
...
void Profile::copyData(Profile* other) {
    ...
    for (QList<Sample*>::iterator it = other->getFirstSample(); it != other->getLastSample(); it++) {
        Sample* s = createSample((*it)->getId());
